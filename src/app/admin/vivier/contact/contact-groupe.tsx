...
type EtatSms = { texte: string; pin?: string; lienSms?: string; erreur?: boolean };
...
export default function ContactGroupe({
  ...
  function libelleSms(res: ResultatSmsConnexion): EtatSms {
    ...
    switch (res.statut) {
      ...
      case 'LIEN_GENERE':
        return { texte: `📱 à envoyer depuis votre téléphone${activation}`, pin: res.pin, lienSms: res.lienSms };
      default:
        return { texte: `📲 échec : ${res.detail ?? ''}`, pin: res.pin, erreur: true };
    }
  }

  /** SMS de connexion : nouveau PIN généré côté serveur, lien pré-langué, profil activé. */
  function envoyerSms(ids: string[], modeEnvoi: 'SERVEUR' | 'LIEN') {
    const question =
      ids.length > 1
        ? `Envoyer le SMS de connexion à ${ids.length} personnes ? Un NOUVEAU PIN est généré pour chacune (l’ancien ne fonctionnera plus).`
        : 'Envoyer le SMS de connexion ? Un NOUVEAU PIN est généré (l’ancien ne fonctionnera plus).';
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap gap-1.5">
        ...
        >
          📲 SMS de connexion (lien + PIN)
        </button>
      </div>

      {connexion && (
        <div className="card mb-4 bg-[#FFF8E6] text-[13px]">
          <b>SMS de connexion</b> : lien dans la langue du profil, téléphone pré-rempli et{' '}
          ...
          {!smsConfigure && (
            <span className="mt-1 block text-muted">
              Envoi serveur non configuré (Paramètres → SMS Twilio) : utilisez « depuis mon
              téléphone », qui ouvre votre app SMS avec le message et le PIN pré-remplis.
            </span>
          )}
        </div>
      )}
                  ...
                  {connexion ? (
                    <>
                      ...
                      <button
                        ...
                        disabled={pending}
                        className={`btn-sm ${smsConfigure ? 'btn-green' : 'btn-outline'}`}
                        title={smsConfigure ? 'Envoi par le serveur (Twilio)' : 'SMS non configuré : simulation'}
                      >
                        📲 Envoyer le SMS
                      </button>
                    </>
